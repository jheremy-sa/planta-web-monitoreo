...
const TagManager = (function () {
  ...
  ═══════════════════════════════════════════════════════════ */
  const SYSTEM_TAGS = [];  // Sin tags hardcodeados: cada planta arranca vacía
  // y sus tags provienen de la tabla `variables` del backend (loadFromVariables).
  ...
  return {
    ...
    /* ─── Cargar tags desde el backend (tabla `variables`) ─ */
    // Fuente de verdad compartida por plant_id: todos los roles con
    // acceso ven exactamente los mismos tags. Reemplaza las defs locales.
    loadFromVariables(plantaId, variables) {
      _plantaId = String(plantaId);
      _defs = {}; _values = {};
      const MAP = { float:'Float', int:'Integer', bool:'Boolean', string:'String' };
      (variables || []).forEach(v => {
        const tipo = MAP[v.data_type] || 'Float';
        const permisos = (v.direction === 'output' || v.role === 'actuator' || v.role === 'setpoint') ? 'rw' : 'r';
        _defs[v.name] = {
          nombre: v.name,
          descripcion: v.unit ? (v.role + ' (' + v.unit + ')') : (v.role || ''),
          grupo: 'Planta', tipo,
          valorInicial: TIPOS[tipo]?.default ?? 0,
          min: (v.min_val ?? null), max: (v.max_val ?? null),
          unidad: v.unit || '', permisos,
          historico: true, alarmas: false, retentivo: false,
          visible: true, activo: true, frecuencia: 1000,
          variableId: v.id, role: v.role, direction: v.direction,
        };
        _values[v.name] = _coerce(tipo, _defs[v.name].valorInicial);
      });
      this._save();   // persistir la verdad del backend (elimina tags viejos guardados)
      return Object.keys(_defs).length;
    },
...
// Exponer en window para que otros scripts (widget-library.js) usen el desplegable de tags
