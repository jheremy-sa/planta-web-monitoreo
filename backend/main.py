...
def listar_plantas(_=Depends(obtener_usuario_actual), db: Session = Depends(get_db)):
    return db.query(models.Planta).filter(models.Planta.is_active == True)\
             .order_by(models.Planta.created_at.desc()).all()
...
def crear_planta(datos: schemas.PlantaCreate,
    ...
    p = models.Planta(nombre=datos.nombre, descripcion=datos.descripcion, created_by=u.email)
    db.add(p); db.commit(); db.refresh(p)
    log(db, u.email, "crear_planta", f"Creó planta: {datos.nombre}")
    return p

@app.get("/api/plantas/{pid}", response_model=schemas.PlantaOut)
def obtener_planta(pid: int, _=Depends(obtener_usuario_actual), db: Session = Depends(get_db)):
    p = db.query(models.Planta).filter(models.Planta.id == pid, models.Planta.is_active == True).first()
...
def guardar_layout_planta(pid: int, datos: schemas.PlantaUpdate,
                          u=Depends(obtener_usuario_actual), db: Session = Depends(get_db)):
    """Guardar el diseño HMI de una planta específica."""
    if u.role != "admin_pro":
        pt = db.query(models.PermisoTemporal).filter(
            models.PermisoTemporal.para_usuario == u.email,
            models.PermisoTemporal.permiso == "editor",
        ...
        if not pt: raise HTTPException(403, "Sin permisos para editar esta planta")
    p = db.query(models.Planta).filter(models.Planta.id == pid, models.Planta.is_active == True).first()
    if not p: raise HTTPException(404, "Planta no encontrada")
    if datos.layout_json is not None:
        p.layout_json = datos.layout_json
    ...
    p.updated_at = datetime.utcnow()
    db.commit(); db.refresh(p)
    log(db, u.email, "guardar_layout", f"Guardó HMI planta #{pid} ({p.nombre})")
