...
# ─── Tokens de DISPOSITIVO (FASE 1) ───────────────────────────────
# El token en claro se muestra UNA sola vez al crear/regenerar el
# dispositivo. En la base solo se guarda su hash SHA-256 (determinista,
# para poder buscar por hash) y un prefijo para identificarlo.
DEVICE_TOKEN_PREFIX_LEN = 8


def generar_device_token() -> str:
    """Token en claro del dispositivo (URL-safe, ~43 chars)."""
    return secrets.token_urlsafe(32)


def hash_device_token(token: str) -> str:
    """SHA-256 hex del token. Determinista => permite lookup por hash."""
    return hashlib.sha256(token.encode("utf-8")).hexdigest()


def prefijo_device_token(token: str) -> str:
    """Prefijo legible del token para mostrarlo tras la creación."""
    return token[:DEVICE_TOKEN_PREFIX_LEN]
